...
def test_db_adapter_general():
    ...
    for use_case_merges in [True, False]:
        # test ops path
        transform_as_data = treatment.description_matrix()
        ops = as_data_algebra_pipeline(
...
def test_db_adapter_monster():
    ...
    ops_cdata = as_data_algebra_pipeline(
        source=descr(d_app=d_app.loc[:, columns]),
        vtreat_descr=transform_as_data,
        treatment_table_name="transform_as_data",
        use_cdata_merges=True,
    ...
    d_app_res_cdata = ops_cdata.eval({"d_app": d_app, "transform_as_data": transform_as_data})
    assert data_algebra.test_util.equivalent_frames(d_app_treated, d_app_res_cdata)